...
# The xymonlaunch snippets go to /etc/xymon/clientlaunch.d, the client's
# own drop-in directory - not tasks.d, which belongs to the server.
set -u
...
for script in preinst postinst postrm; do
    cp "packaging/deb/$script" "$ROOT/DEBIAN/$script" || exit 1
